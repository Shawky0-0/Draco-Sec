...
async def create_scan(
    ...
    if current_user.scans_remaining <= 0:
        raise HTTPException(
            status_code=403,
            detail=f"You have reached your limit of offensive scans for the {current_user.plan_tier} plan."
        )
        
    # Deduct a scan
    current_user.scans_remaining -= 1
    # Note: We don't need a separate db.add(current_user) as it's already attached to the session
    # Create scan record
    scan = OffensiveScan(
...
def get_scan_report(
    ...
    """Get the final pentest report (markdown) generated by Strix.
    
    Falls back to extracting the report from AgentEvent rows if final_report_md
    was not saved (e.g. because Strix emitted it as raw XML in the feed).
    """
    ...
    report_md = scan.final_report_md

    # ── Fallback: extract report from agent events ──────────────────────────
    # Only runs once — the result is persisted so subsequent calls are instant.
    if not report_md:
        # DB-level LIKE filter — avoids pulling all events into Python memory.
        # Search from the END (finish_scan is always in the last few events).
        candidate_events = (
            db.query(AgentEvent)
            .filter(
                AgentEvent.scan_id == scan_id,
                AgentEvent.content.like('%finish_scan%')
            )
        ...
        for ev in candidate_events:
            try:
                data = _json.loads(ev.content) if isinstance(ev.content, str) else ev.content
            except Exception:
                data = ev.content

            if ev.event_type == "tool_start" and isinstance(data, dict):
                if data.get("tool") in ("finish_scan", "finish_action"):
                    args = data.get("args", {})
                    report_md = args.get("content") or args.get("report") or args.get("message")
                    if report_md:
                        break

            if ev.event_type == "chat":
                text = data.get("content", "") if isinstance(data, dict) else str(data)
                # <parameter=content>…</parameter>  (Strix raw XML format)
                m = _re.search(
                    r'<parameter=content>([\s\S]+?)(?:</parameter=content>|</parameter>)',
                    text, _re.IGNORECASE
                )
                if not m:
                    m = _re.search(r'<parameter\s+name=["\']?content["\']?>([\s\S]+?)</parameter>', text, _re.IGNORECASE)
                if not m:
                    m = _re.search(r'<finish_scan[^>]*>([\s\S]+)', text, _re.IGNORECASE)
                if m:
                    report_md = m.group(1).strip()
                    break

        # Persist — next call returns immediately from scan.final_report_md
        if report_md:
            scan.final_report_md = report_md
            if scan.status in ("running", "failed", "stopped"):
                scan.status = "completed"
                scan.completed_at = scan.completed_at or _dt.utcnow()
            db.commit()
    # ────────────────────────────────────────────────────────────────────────
...
def list_methodologies(
    ...
    """List all methodologies: system defaults + current user's custom ones."""
...
@router.get("/methodologies/defaults", response_model=List[MethodologyResponse])
def list_default_methodologies(
    db: Session = Depends(get_db)
):
    """Public endpoint: list only system default methodologies (no auth required)."""
    methodologies = db.query(ScanMethodology).filter(
        ScanMethodology.is_default == 1
    ).order_by(ScanMethodology.created_at.asc()).all()
    return methodologies
